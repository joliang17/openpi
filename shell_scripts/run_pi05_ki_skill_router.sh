#!/bin/bash
#SBATCH --job-name=pi05_ki_skill_router
#SBATCH --output=slurm_output/pi05_ki_skill_router_%j.log
#SBATCH --error=slurm_output/pi05_ki_skill_router_%j.log
...
export KI_BASE_PARAMS="${KI_BASE_PARAMS:-/fs/nexus-projects/wilddiffusion/vla/openpi_ki/pi05_libero_ki_vlm_lora_action_expert/pi05_ki_vlm_lora_action_expert_20260509_120950/29999/params}"
if [[ ! -d "${KI_BASE_PARAMS}" ]]; then
  echo "KI base params directory not found: ${KI_BASE_PARAMS}" >&2
  exit 1
fi

CHECKPOINT_BASE_DIR="${CHECKPOINT_BASE_DIR:-/fs/nexus-projects/wilddiffusion/vla/openpi_ki_skill_router}"
RUN_TS="${RUN_TS:-$(date +%Y%m%d_%H%M%S)}"

STAGE1_CONFIG="pi05_libero_ki_skill_router_stage1"
STAGE2_CONFIG="pi05_libero_ki_skill_router_stage2"
STAGE1_EXP_PROVIDED="${STAGE1_EXP:-}"
STAGE1_EXP="${STAGE1_EXP:-pi05_ki_skill_router_stage1_${RUN_TS}}"
STAGE2_EXP_PROVIDED="${STAGE2_EXP:-}"
STAGE2_EXP="${STAGE2_EXP:-pi05_ki_skill_router_stage2_from_${STAGE1_EXP}_${RUN_TS}}"
