#!/bin/bash
#SBATCH --job-name=pi05_vlm_lora_sr_llm
#SBATCH --output=slurm_output/pi05_vlm_lora_skill_router_joint_llm_skill_init_%j.log
#SBATCH --error=slurm_output/pi05_vlm_lora_skill_router_joint_llm_skill_init_%j.log
...
CONFIG_NAME="${CONFIG_NAME:-pi05_libero_vlm_lora_skill_router_joint_llm_skill_init}"
RUN_TS="${RUN_TS:-$(date +%Y%m%d_%H%M%S)}"
EXP_NAME="${EXP_NAME:-pi05_vlm_lora_skill_router_joint_llm_skill_init_${RUN_TS}}"
CHECKPOINT_BASE_DIR="${CHECKPOINT_BASE_DIR:-/fs/nexus-projects/wilddiffusion/vla/openpi_vlm_lora_skill_router_llm_skill_init}"
