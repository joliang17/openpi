#!/bin/bash
# Evaluate a running pi05_action_expert_jax OpenPI policy server on LIBERO-10 and LIBERO-PRO.
set -euo pipefail
...
HOST="${HOST:-localhost}"
PORT="${PORT:-8010}"
EXP_TAG="${EXP_TAG:-pi05_libero_skill_router_stage2_adarms_only}"
# SUITES="${SUITES:-libero10 libero_pro}"
SUITES="${SUITES:-libero_pro}"
...
echo "[$(date -Iseconds)] Evaluating pi05_action_expert_jax server ${HOST}:${PORT}"
