...
#SBATCH --job-name=eval_srj_shuf
#SBATCH --output=slurm_output/eval_pi05_skill_router_joint_shuffle_%j.log
#SBATCH --error=slurm_output/eval_pi05_skill_router_joint_shuffle_%j.log
...
# Shuffled-skill-embedding ablation: serves the trained joint checkpoint with
# config pi05_libero_vlm_lora_skill_router_joint_shuffle_eval, which randomly
# permutes the skill embedding bank at inference. Compare its success rate
# against eval_pi05_skill_router_joint.sh to test whether the skill routing
...
EXP_NAME="pi05_skill_router_joint_shuffle"
POLICY_CONFIG="pi05_libero_vlm_lora_skill_router_joint_shuffle_eval"
PORT="${PORT:-8027}"
...
# Wait 10 min so the policy server fully cold-starts (JAX JIT + warm-up)
# before the eval client begins calling it.
SERVER_READY_WAIT="${SERVER_READY_WAIT:-600}"
