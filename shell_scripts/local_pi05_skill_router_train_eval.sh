...
STAGE2_EXP="${STAGE2_EXP:-pi05_skill_router_stage2_from_stage1_20260507_101717_step6000_20260508_152339}"
STAGE2_NUM_TRAIN_STEPS="${STAGE2_NUM_TRAIN_STEPS:-30000}"
STAGE2_RESUME="${STAGE2_RESUME:-1}"
...
echo "[$(date -Iseconds)] === Starting training: ${STAGE2_EXP} (resume=${STAGE2_RESUME}) ==="
...
############################################
STAGE2_DIR="${CHECKPOINT_BASE_DIR}/pi05_libero_skill_router_stage2/${STAGE2_EXP}"
...
python3 -u scripts/serve_policy.py \
  --port="${PORT}" \
  policy:checkpoint \
  --policy.config=pi05_libero_skill_router_stage2 \
  --policy.dir="${CKPT}" \
